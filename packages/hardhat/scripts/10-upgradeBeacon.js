...
const deploy = async (contractName, _args = [], overrides = {}, libraries = {}) => {
  ...
  const beaconAddress = "0xE27fDAAD604Dc703598Afa8A577714f94EB578Ce";

  const Box = await ethers.getContractFactory(contractName);
  const box = await upgrades.prepareUpgrade(beaconAddress, Box); // unsafeAllowCustomTypes:true
  ...
  fs.writeFileSync(`artifacts/${contractName}NewProxy.address`, deployed.address);
