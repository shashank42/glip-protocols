...
// MUMBAI
  ...
  const auctionProxy = "0x4b2C97dD578912b730cb04f3Bcc5F3d86d197c0c";
  ...
  let overrides2 = { initializer: '__ERC1155GlipPass_init', timeout: 0 };
  contractArgs = ["GlipBox", "GLIPBOX", "https://be.namasteapis.com/metadata/v1/", "", auctionProxy, minterProxy, metatransactionForwarderContract];
  var {beacon: assetBeacon, beaconImpl: assetBeaconImpl, proxy1: globalAssetProxy} = await deployTokenBeaconAndBeaconProxy("ERC1155GlipPass", contractArgs, overrides1, overrides2, passContractOwner, {});
  
  console.log({minterProxy, auctionProxy, assetBeacon, assetBeaconImpl, globalAssetProxy});
  ...
  var factoryAddress = await deployTokensFactory ("ERC1155GlipPassFactoryC2", contractArgs, {}, {});


  // MANUAL OVERRIDES

  // TEST-PROXY CONTRACT
//   overrides = { initializer: '__ERC1155GlipPass_init', timeout: 0};
//   contractArgs = ["Glip", "GLIP", true, "ipfs://", "", auctionProxy, minterProxy];
//   const testTokenBeaconProxy = await deployTokenBeaconProxy ("ERC1155GlipPass", assetBeacon, contractArgs, overrides, {})
...
const deployTokenBeaconAndBeaconProxy = async (contractName, _args = [], overrides1 = {}, overrides2 = {}, passContractOwner, libraries = {}) => {
    ...
    // Verifying contract
    console.log("Verifying contract");
    exec(`npx hardhat verify --contract contracts/tokens/erc-1155/ERC1155GlipPass.sol:ERC1155GlipPass --network ${defaultNetwork} ${beaconImplAddress}`, (error, stdout, stderr) => {
      if (error) {
          console.log(`error: ${error.message}`);
          return;
      }
...
const deployTokensFactory = async (contractName, _args = [], overrides = {}, libraries = {}) => {
    ...
    // Verifying contract
    console.log("Verifying contract");
    exec(`npx hardhat verify --contract contracts/tokens/create-2/ERC1155GlipPassFactoryC2.sol:ERC1155GlipPassFactoryC2 --network ${defaultNetwork} ${box.address} ${contractArgs[0]} ${contractArgs[1]} ${contractArgs[2]}`, (error, stdout, stderr) => {
      if (error) {
          console.log(`error: ${error.message}`);
          return;
      }
