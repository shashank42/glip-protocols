...
const main = async () => {
  ...
  const yourCollectible = await deploy("AuctioneerUpgradable") // <-- add in constructor args like line 19 vvvv
...
const deploy = async (contractName, _args = [], overrides = {}, libraries = {}) => {
  ...
  const fee = 100;
  contractArgs = [auctioneer, fee];
  

  const Box = await ethers.getContractFactory(contractName);
  const box = await upgrades.deployProxy(Box, contractArgs, { initializer: '__AuctioneerUpgradable_init', unsafeAllow: ["delegatecall"]  }); // unsafeAllowCustomTypes:true
  ...
  exec(`npx hardhat verify --contract contracts/roles/AuctioneerUpgradable.sol:AuctioneerUpgradable --network mumbai ${implAddress}`, (error, stdout, stderr) => {
    if (error) {
