...
const deploy = async (contractName, _args = [], overrides = {}, libraries = {}) => {
  ...
  const splitterImplementationContract = "0x9ecEC7e112dD24474C5BE054b782F574E55011EE";
  const fee = 100;
  const defaultRoyalty = 250;
  contractArgs = [minter, fee, defaultRoyalty, splitterImplementationContract];
  
  const Box = await ethers.getContractFactory(contractName);
  const box = await upgrades.deployProxy(Box, contractArgs, { initializer: '__MinterUpgradable_init', unsafeAllow: ["delegatecall"] }); // unsafeAllowCustomTypes:true
  await box.deployed();
  ...
  exec(`npx hardhat verify --contract contracts/roles/MinterUpgradeable.sol:MinterUpgradeable --network mumbai ${implAddress}`, (error, stdout, stderr) => {
    if (error) {
